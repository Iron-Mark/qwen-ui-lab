# Post-Launch Guide (Operators)

Use this checklist after the public app is live. The product stays **local-analysis-first**: no API key or live Qwen flag is required for presentations.

## Local-analysis defaults (do not change for public walkthroughs)

| Concern | Default | Why |
|--------|---------|-----|
| Analysis | `QWEN_LIVE_ANALYSIS` unset | Instant local analysis; no API spend |
...
See **[DEMO.md](../DEMO.md)** for the sample-run click path and **[docs/ops/PRODUCTION_DEPLOY_LANE.md](./PRODUCTION_DEPLOY_LANE.md)** for deploy gates.

## Pre-walkthrough verification (5 minutes)
...
Violations are **report-only** in production - they do not block users.
...
3. **Rollout:** see **[docs/ops/CSP_HARDENING_GUIDE.md](./CSP_HARDENING_GUIDE.md)** - keep `CSP_REPORT_ONLY=false` off until violations are triaged; never tighten enforced policy during a public walkthrough without a rehearsal build.

## Incident quick path

1. `GET /api/health` - expect local-analysis mode on the public app.
