...
Use this sequence to rehearse live vision on a **Preview** deployment without changing **Production** env (local-analysis-safe default).
...
| `QWEN_MODEL` | Optional (ignored in local-analysis mode) | **Required** - e.g. `qwen3-vl-plus` | Same as preview |
...
**Production copy-paste block (local-analysis safe + platform ops):**
...
2. Leave **Production** `QWEN_LIVE_ANALYSIS` unset for the local-analysis-safe default.
...
- **Local analysis:** live flag off -> **no rate limit** on this route (client usually skips the route; server may still return a local-analysis artifact if called).
...
| Local-analysis host, no live flag | App should **not** rate-limit this route | If you see 429, confirm `GET /api/health` -> `liveAnalysisEnabled: false` |
...
Do **not** raise limits on **Production** until you intentionally run Stage C and accept abuse risk. For public local-analysis production, keep live off - no rate limit applies.
...
See also **[TROUBLESHOOTING_RUNBOOK.md](./TROUBLESHOOTING_RUNBOOK.md)** section  live / 429.
...
**Local-analysis gate (default CI / local, no live env):**
...
- `--target=local` with `QWEN_LIVE_ANALYSIS=true` -> exit `1`
...
1. **Stage A - Preview**
   ...
   - [ ] Local-analysis baseline on preview (if env not yet live): `DEPLOY_URL=<preview-url> npm run smoke:deploy`
...
Full release checklist: **[DEPLOYMENT_CHECKLIST.md](./DEPLOYMENT_CHECKLIST.md)** section 2-5.
...
- `staged-live-smoke.mjs`: HTTPS only; blocks the public production host unless `ALLOW_PRODUCTION_LIVE_SMOKE=1`
...
## Rollback to local-analysis mode (fast containment)

Use this when live vision misbehaves, quota spikes, or you need local-analysis behavior **without** redeploying code.
...
2. **Verify local-analysis policy locally** (optional, with live vars unset first):
   ```bash
   npm run deploy:env:local
   ```
   Expected: exit `0`, `Live analysis requested: no`.
3. **Verify deployed host is local-analysis safe**:
   ```bash
...
## Public Local-Analysis Reminder

Operators should keep **`QWEN_LIVE_ANALYSIS` unset** on the public Vercel project unless a live-analysis rollout is intentional. See **[POST_LAUNCH.md](./POST_LAUNCH.md)** and **[DEMO.md](../DEMO.md)**.
