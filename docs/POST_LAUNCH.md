...
| CSP enforce | Stage A+B in `next.config.ts` (`connect-src` HTTPS-only, `upgrade-insecure-requests`, script without `unsafe-eval`; style still allows `unsafe-inline`) | Avoid breaking Next/Tailwind runtime |
