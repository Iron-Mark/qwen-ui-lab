...
`src/lib/share-store.mjs` keeps an in-memory `Map` per Node/serverless instance. Links survive until TTL expiry (default 7 days) but may not resolve across cold starts or other instances.
...
- See `docs/PRODUCTION_DEPLOY_LANE.md` for general Vercel env policy (`NEXT_PUBLIC_*` must not carry secrets).
