...
AI-assisted UI scaffolding: turn UI screenshots into React and Tailwind starting points you can refine quickly.
...
Demo mode is the default. Live Qwen analysis requires both:
...
| `/demo` | One-click offline demo with bundled references and `?archetype=auth|mobile|landing|settings|shop`. |
| `/design-system` | Component catalog, UX-law domains, snippet previews, and export bundle. |
| `/account` | Demo-safe guest/display-name/magic-link stub stored in `sessionStorage`. |
...
Public meetup deployments should keep `QWEN_LIVE_ANALYSIS` unset. In demo mode the client uses deterministic offline artifacts, so stage demos and CI do not spend Qwen credits.
...
| [Demo](./DEMO.md) | Presenter flow and troubleshooting table. |
...
| Doc | Topic |
| --- | --- |
| [Meetup media](./media/MEETUP_MEDIA.md) | Recording and slide export checklist. |
...
| [Meetup slides](./media/MEETUP_SLIDES.marp.md) | Marp slide source. |
...
| [Security](./ops/SECURITY.md) | Responsible reporting and safe demo operations. |
...
| Doc | Topic |
| --- | --- |
| [Artifact checklist](./specs/ARTIFACT_CHECKLIST.md) | Demo artifact inventory. |
