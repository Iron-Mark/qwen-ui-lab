...
|-------|----------|----------|
| State machine | `src/lib/auth.mjs` | Modes: `guest` → `named` (display name) or `magic-link-pending` → `named` |
| Persistence | `sessionStorage` key `qwen-ui-lab:auth` | Tab-scoped; cleared on sign-out or empty name |
| React context | `src/lib/auth.tsx` | `AuthProvider` wraps the app in `layout.tsx` |
| UI | `src/app/account/AccountPageClient.tsx` | Display name form + magic-link stub (confirm without token) |
...
Mirror **[docs/PRODUCTION_DEPLOY_LANE.md](./PRODUCTION_DEPLOY_LANE.md)** — demo-safe by default, live auth opt-in.
...
- [ ] CSP updates — see **[docs/CSP_HARDENING_GUIDE.md](./CSP_HARDENING_GUIDE.md)** for auth callback origins
- [ ] No PII in analytics events — see **[docs/ANALYTICS_TAXONOMY.md](./ANALYTICS_TAXONOMY.md)**
...
- **[docs/PRODUCTION_DEPLOY_LANE.md](./PRODUCTION_DEPLOY_LANE.md)** — demo vs live env policy (model for `AUTH_ENABLED`)
- **[docs/CSP_HARDENING_GUIDE.md](./CSP_HARDENING_GUIDE.md)** — CSP when adding auth callbacks
- **[docs/OFFLINE_DEMO_E2E.md](./OFFLINE_DEMO_E2E.md)** — offline guarantees E2E must preserve
